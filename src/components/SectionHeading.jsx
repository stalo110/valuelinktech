function SectionHeading({ eyebrow, title, description, center = false, light = false }) {
  const headingClass = light ? 'text-white' : 'text-slate-900 dark:text-white';
  const copyClass = light ? 'text-slate-200' : 'text-slate-600 dark:text-slate-300';
  const eyebrowClass = light ? 'bg-white/10 text-slate-100' : 'bg-teal/10 text-teal dark:bg-teal/10 dark:text-teal';
